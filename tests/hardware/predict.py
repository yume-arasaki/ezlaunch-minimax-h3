...
def predict(name: str, vram_mib: int, profile: str | None) -> dict:
    ...
    if profile == "dgx_spark":
        return {
            ...
            "arch": arch,
            "bw_gb_s": bw,
            "note": "DGX Spark GB10 unified 128GB. PRIMARY recipe: drowzeys Sol-Engine (1.54x, fixes SM121 miscode) — 10s @ 720p 20-step ~20 min (M ours); 5s@864x480 202.5s (M drowzeys); 15s/362f ~32 min. sage<3.0, no --use-sage-attention. disk ≥200GB.",
        }
